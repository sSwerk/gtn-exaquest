...
$testMessage = XlsImporter::test(); // quick and dirty test whether autoload was successful
...
$url = new moodle_url('/blocks/exaquest/similarity_comparison.php', $pageParams);
$PAGE->set_url($url);
...
echo $output->header($courseContext, $courseID, get_string('exaquest:similarity_title', 'block_exaquest'));
...
if (is_enrolled($courseContext, $USER, "block/exaquest:createquestion")) {
    // ADD QUESTION
...
if (is_enrolled($courseContext, $USER, "block/exaquest:modulverantwortlicher")) {
...
if (is_enrolled($courseContext, $USER, "block/exaquest:fragenersteller")) {
        // QUESTIONS TO REVIEW
